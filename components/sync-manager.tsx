...
import { Cloud, CloudOff, Download, Upload, RefreshCw, CheckCircle, AlertTriangle, Info, HardDrive } from "lucide-react"
...
export function SyncManager({ onExport, onImport, taskCount }: SyncManagerProps) {
  const [isExporting, setIsExporting] = useState(false)
  const [isImporting, setIsImporting] = useState(false)
  const [lastBackup, setLastBackup] = useState<Date | null>(() => {
    const saved = localStorage.getItem("last-backup-time")
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            {syncStatus !== "syncing" && syncMessage && (
              ...
              >
                <AlertDescription className={getStatusColor()}>{syncMessage}</AlertDescription>
              </Alert>
            )}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
              <div>
                ...
                <div className="text-sm text-muted-foreground">{formatLastBackup(lastBackup)}</div>
              </div>
              <Badge
                variant="outline"
                className={cn(
                  backupRecommendation === "urgent" && "text-red-600 border-red-200",
                  backupRecommendation === "recommended" && "text-amber-600 border-amber-200",
                  backupRecommendation === "good" && "text-green-600 border-green-200",
                )}
              >
            ...
            {backupRecommendation === "urgent" && (
              <Alert className="border-red-200 bg-red-50">
                ...
                <AlertDescription className="text-red-700">
                  Bạn chưa sao lưu dữ liệu hoặc đã lâu không sao lưu. Hãy tạo bản sao lưu để bảo vệ dữ liệu của bạn.
                </AlertDescription>
              </Alert>
            )}
        ...
        <CardContent>
          <div className="space-y-3 text-sm text-muted-foreground">
            <div className="flex items-start gap-2">
              ...
              <div>
                <strong>Lưu trữ cục bộ:</strong> Dữ liệu được lưu trực tiếp trên trình duyệt của bạn. Dữ liệu sẽ mất nếu
                xóa cache hoặc dữ liệu trình duyệt.
              </div>
            </div>
            <div className="flex items-start gap-2">
              <Info className="h-4 w-4 mt-0.5 flex-shrink-0" />
              <div>
                <strong>Sao lưu:</strong> Tạo file JSON chứa toàn bộ dữ liệu nhiệm vụ để lưu trữ an toàn hoặc chuyển
                sang thiết bị khác.
              </div>
            </div>
            <div className="flex items-start gap-2">
              <Info className="h-4 w-4 mt-0.5 flex-shrink-0" />
              <div>
                <strong>Khôi phục:</strong> Nhập file JSON để khôi phục dữ liệu từ bản sao lưu. Thao tác này sẽ ghi đè
                lên dữ liệu hiện tại.
              </div>
            </div>
            <div className="flex items-start gap-2">
              <Info className="h-4 w-4 mt-0.5 flex-shrink-0" />
              <div>
                <strong>Đồng bộ cloud:</strong> Tính năng này hiện đang trong giai đoạn phát triển. Sẽ cho phép đồng bộ
                dữ liệu qua nhiều thiết bị.
              </div>
            </div>
          </div>
        </CardContent>
