...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.pool.query('INSERT INTO users(email, name, password) VALUES($1,$2,$3) RETURNING id, email, name, created_at', [email, resolvedName, pwd]);
        const user = {
            id: result.rows[0].id,
        ...
        const token = jsonwebtoken_1.default.sign({ userId: user.id }, process.env.JWT_SECRET || 'dev_jwt', { expiresIn: '7d' });
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ userId: userRow.id }, process.env.JWT_SECRET || 'dev_jwt', { expiresIn: '7d' });
        const user = {
            id: userRow.id,
            email: userRow.email,
...
router.get('/me', async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.pool.query('SELECT id, email, name, created_at FROM users WHERE id=$1', [payload.userId]);
        if (result.rowCount === 0)
            return res.json({ success: true, user: null });
